...
my_matrix <- matrix(NA, nrow = 30, ncol = 3)
# use for loop to run 30 iterations
for(i in 1 : 30) {
  # use another for loop to iterate through three values of k
  for(j in 1 : 3) {
    # run function using k=2
    mse2 <- my_rf_cv(2)
    # store mse outcome in my_matrix
    my_matrix[i, 1] <- mse2
...
#mse2_mean <- mean(my_matrix[,1])
#mse2_sd <- sd(my_matrix[,1])
# calculate the mean and sd of the 30 simulations in k=5
#mse5_mean <- mean(my_matrix[,2])
#mse5_sd <- sd(my_matrix[,2])
# calculate the mean and sd of the 30 simulations in k=10
#mse10_mean <- mean(my_matrix[,3])
#mse10_sd <- sd(my_matrix[,3])

# create data frame to hold columns from my matrix
#my_data <- data.frame("k=2" = my_matrix[, 1],
#                      "k=5" = my_matrix[, 2],
#                      "k=10" = my_matrix[, 3])

# create 3 boxplots with a boxplot associated with each value of k
#my_boxplot <- ggplot(my_data, aes(x = Actual, y = Fitted, color = Continent)) + 
#  geom_boxplot() + 
#  labs(title = "") + 
  #theme_bw(base_size = 12) + 
#  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
